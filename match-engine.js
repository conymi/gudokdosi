...
function renderSokchoCourse(relation){
  ...
  return `
    <div class="course-card">
      ...
      <div class="cv-title">${c.title}</div>
...
function showResult(){
  ...
  const courseBlock = sokchoIsTop
  ? `<div class="r-section topgap">먼저 열리는 도시 · 속초</div>
     ${renderSokchoCourse(final.relation)}`
  : `<div class="r-section topgap">${top1.name}의 하루</div>
     <div class="course-card">
       <div class="cv-promise">${top1.name}의 하루 동선은 지금 준비 중이에요.<br>
       <b>가장 먼저 열리는 속초부터, 곧 그 동네 사람의 하루까지.</b></div>
     </div>`;

  ovCount.textContent = "결과";
  ovDots.innerHTML="";
  const ctaNote = sokchoIsTop
    ? `당신과 가장 잘 맞는 도시는 <b>속초</b>.<br>준비가 끝나면 가장 먼저 알려드릴게요.`
    : `당신과 가장 잘 맞는 도시는 <b>${top1.name}</b>.<br>다른 도시도 곧 깊어져요. 가장 먼저 열리는 건 <b>속초</b> — 준비되면 알려드릴게요.`;
