...
const USER_TYPE_POOL = {
  ...
  "일상 여행자형":       {desc:"사람과 분위기 속에 섞이는, 일상 같은 여행."},
};
...
function getUserType(a){
  ...
  if(together&&(nature||sp>=0.5)) return ulabel(slow?"나란히 걷는 해변형":"일상 여행자형");
...
const CITY_SCORES = [
  ...
  {city_id:"seogwipo",name:"서귀포",axis:{tempo:0.467,consume:0.514,space:0.413,relation:0.619,infra:0.878},
   confidence:"mid",cityType:"일상 여행자형",oneLiner:"일상의 아름다움을 함께 나누는 곳",
...
const CITY_COURSES = {
  gongju: {
    ...
    steps: [
      ...
      {name:"무령왕릉", dur:"낮",   comment:"고즈넉해요. 백제가 어떤 시간이었는지, 여기서 조용히 스쳐 지나가요."},
  ...
  tongyeong: {
    ...
    steps: [
      {name:"동피랑마을", dur:"아침", comment:"언덕을 올라서면 골목골목 벽화가 나와요. 정감 있어요."},
      {name:"강구안",     dur:"낮",   comment:"바다가 늘 잔잔하고 예뻐요. 충무김밥이랑 꿀빵, 여기서 먹어봐요."},
  ...
  seogwipo: {
    ...
    steps: [
      {name:"천지연폭포", dur:"아침", comment:"도심에서 가까워서 가볍게 들러요. 아담한데 또 웅장해요."},
      {name:"올레시장",   dur:"낮",   comment:"잘 단장된 시장이라 먹을 게 많아요. 이중섭거리랑 이어져서 같이 돌기 좋아요."},
      {name:"외돌개",     dur:"저녁", comment:"바다 절벽에 장승처럼 바위가 솟아 있어요. 제주의 숨은 비경이에요."},
    ]
  },
};
...
function bindMatchEngine(){
  ...
  const sb = document.getElementById('signupBtn');   if(sb) sb.href = CONFIG.formUrl;
  const sb2 = document.getElementById('signupBtn2');  if(sb2) sb2.href = CONFIG.formUrl;
...
if(sokchoIsTop){
  ...
  ovContent.innerHTML = `
    <div class="result q-anim">
      <div id="shareCapture">
        <div class="r-label">나의 여행 유형</div>
      ...
      ${courseBlock}
        ...
        <a class="btn btn-fill" href="${CONFIG.formUrl}" target="_blank" rel="noopener">이 결과, 어떠셨나요?</a>
        <button class="r-share" onclick="shareResult('${top1.name}', ${top1.matchPct})">결과 이미지로 저장하기</button>
      </div>
    </div>`;
  overlay.scrollTop=0;
}
